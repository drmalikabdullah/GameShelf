...
function renderBigPictureStage() {
  ...
  bpUpdateBackdrop(g);
  // Set hero image as stage background with minimal overlay
  const stagewrap = document.getElementById('bpStage').parentElement;
  if (g && g.hero_url) {
    stagewrap.style.backgroundImage = `linear-gradient(rgba(10, 11, 16, 0.1), rgba(10, 11, 16, 0.1)), url('${escapeHtml(heroUrl(g))}')`;
  } else {
